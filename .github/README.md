...
jobs:
  dependabot:
    uses: France-Travail/pass-emploi-tools/.github/workflows/dependabot.yml@{ref}
```
...
jobs:
  security-alerts:
    uses: France-Travail/pass-emploi-tools/.github/workflows/security-alerts.yml@{ref}
    with:
      repo-name: 'pass-emploi-api'  # Nom affiché dans le message Mattermost
    secrets:
      DEPENDABOT_ALERTS_READER: ${{ secrets.DEPENDABOT_ALERTS_READER }}
...
jobs:
  security-checks:
    uses: France-Travail/pass-emploi-tools/.github/workflows/security-checks.yml@{ref}
```
...
```yaml
steps:
  - uses: France-Travail/pass-emploi-tools/.github/actions/setup-node-yarn@{ref}
    with:
...
jobs:
  sonarqube:
    ...
    steps:
      - uses: France-Travail/pass-emploi-tools/.github/actions/sonarqube-scan@{ref}
        with:
...
3. Merger sur `master`
4. Modifier le hash de commit dans les repos appelants
